...
const reducer = (state, action) => {
  switch (action.type) {
    case DELETE_TASK: {
      let idx = state.findIndex((task) => task.id === action.id);
      if (idx >= 0) {
    ...
    case UPDATE_TASK: {
      let idx = state.findIndex((task) => task.id === action.task.id);
      if (idx >= 0) {
        return state.splice(idx, 1, action.task);
      }
      return state;
    }
    case CREATE_TASK: {
...
export const useTask = () => useContext(TaskProviderContext);
